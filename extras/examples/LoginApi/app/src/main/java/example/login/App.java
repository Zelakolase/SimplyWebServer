    ...
    try {
      if (httpRequest.getPath().equals("/index.html")) {
        ...
        HashMap<String, String> LoginInfo =
            JSON.QHM(httpRequest.getBodyAsString());
        if (!(LoginInfo.containsKey("username") &&
              LoginInfo.containsKey("password"))) {
          httpBufferResponse.setBody(JSON.HMQ(new HashMap<>() {
            { put("success", "false"); }
          }));
        } else {
          if (LoginInfo.get("username").equals(username) &&
              LoginInfo.get("password").equals(password)) {
            httpBufferResponse.setBody(JSON.HMQ(new HashMap<>() {
              { put("success", "true"); }
            }));
          } else {
      ...
      httpBufferResponse.addHeader("Rand-Int",
                                   String.valueOf(new Random().nextInt(10)));
    } catch (IOException ignored) {
      httpBufferResponse.setHttpStatusCode(
